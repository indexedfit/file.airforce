...
export function renderRoomsList(rooms, onOpen) {
  const ul = $('rooms-list'); if (!ul) return
  const frag = document.createDocumentFragment()
  rooms.forEach(r => {
    const li = document.createElement('li')
    li.className = 'py-2 flex items-center gap-3'
    ...
    li.innerHTML = `
      <div class="flex-1">
        <div class="font-medium">${r.name || '(room)'} · <span class="text-xs text-gray-500">${filesCount} file(s)</span></div>
        <div class="text-xs text-gray-600">Last seen: ${r.lastSeen ? formatDistanceToNow(r.lastSeen, { addSuffix: true }) : '—'}</div>
      </div>
      <div class="flex gap-2">
        <button data-action="open" class="px-2 py-1 border rounded text-sm">Open</button>
      </div>
...
export function renderRoomDetails(room, opts = {}) {
  const root = $('rooms-info'); if (!root) return
  const files = room?.manifest?.files || []
  root.innerHTML = `
    <div class="mb-3">
      <div class="font-semibold">${room?.name || '(room)'} <span class="text-xs text-gray-500">${room?.id || ''}</span></div>
    </div>
    <div class="grid md:grid-cols-2 gap-4">
      <div>
        <div class="font-medium mb-1">Files in room</div>
        ${files.length ? '' : '<div class="text-sm text-gray-600">No files yet.</div>'}
        <ul id="room-files" class="space-y-1">${files.map(f => `
...
export function renderChatMessages(messages, selfId = '') {
  ...
  messages.forEach(m => {
    ...
    div.className = `text-sm mb-1 ${isSelf ? 'text-blue-700' : 'text-gray-800'}`
